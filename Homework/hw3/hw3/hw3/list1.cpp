...
void list::executeCommand(ofstream& write, string command)
{
	if (command == "printList")
		outputList(write, head);
	else if (command == "printListBackwards")
		outputList(write, tail);
...
void list::outputList(ofstream& write, node* start) //called "printList" and "printListBackwards" in instructions
{
	...
	else if (start == head) //outputting in forwards direction
	{
		write << "List:\n";
	...
	{
		write << "Reversed List: \n";
		for (node* cur = start; cur != nullptr; cur = cur->previous)
		{
			write << cur->type << ":" << cur->expression << endl;
		}
	}
...
{
	write << "Top of reserve: " << reserve.top()->type << ":" << reserve.top()->expression << endl << endl;
}
